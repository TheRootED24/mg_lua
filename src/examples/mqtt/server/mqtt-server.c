...
int new_sub (lua_State *L) {
	...
	luaL_getmetatable(L, "LuaBook.sub");
...
static const struct luaL_reg mqtt_serv_methods_f[] = {
{ "new",		new_sub		},
{ "next",		_sub_next	},
{ "c",			_sub_connection	},
{ "topic",		_sub_topic	},
...
static const struct luaL_reg mqtt_serv_methods_m[] = {
{ "new",		new_sub		},
{ "next",		_sub_next	},
{ "c",			_sub_connection	},
{ "topic",		_sub_topic	},
...
int luaopen_mqtt_server(lua_State *L)
{
	lua_newtable(L);
	luaL_register(L, NULL, mqtt_serv_methods_m);
	// mg_str
	luaL_newmetatable(L, "LuaBook.sub");
	...
	//lua_settable(L, -3);  /* metatable.__index = metatable */
	luaL_openlib(L, NULL, mqtt_serv_methods_m, 0);
	luaL_openlib(L, "mg_sub", mqtt_serv_methods_f, 0);
	lua_pop(L, 2);
	/*luaL_newmetatable(L, "LuaBook.sub");
