...
CALLBACK = function(c, ev, ev_data)
	if( ev == MG_EV_HTTP_MSG ) then
		...
		if(mg.string.match(mg.str.ptr(hm.uri), mg.str.ptr("/api/f1"))) then
			local conn = mg.connection.ptr(c);
			mg.http.reply(conn, 200, "Content-Type: application/json\r\n", string.format("{\"result\": %d}", 123));
		elseif(mg.string.match(mg.str.ptr(hm.uri), mg.str.ptr("/api/sum"))) then
			local num1 = mg.json.get_num(mg.str.ptr(hm.body), "$[0]")
			local num2 = mg.json.get_num(mg.str.ptr(hm.body), "$[1]")
...
local function main(...)
	...
	-- Setup listener
	mg.http.listen(mgr, "localhost:8000", "CALLBACK")
