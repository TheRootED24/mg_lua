...
local s_url = "ws://localhost:8000/websocket";
local run = true;
...
WS_CALLBACK = function (c, ev, ev_data)
	...
	elseif (ev == MG_EV_WS_MSG) then
		...
		print(string.format("GOT ECHO REPLY: [ %s ] Length [ %d ]", wm.data, wm.len));
		local conn = mg.connection.new(c);
...
local function main(arg)
	...
	while(i < #arg) do
		...
		elseif(arg[i] == "-url") then
			...
			local payload = string.format(usage, arg[0], s_ca_path, s_url)
	...
	mg.ws.connect(mgr, s_url, "WS_CALLBACK"); 	-- Create client 
