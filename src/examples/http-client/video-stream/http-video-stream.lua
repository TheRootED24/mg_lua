...
server_callback = function (c, ev, ev_data)
	if(ev == MG_EV_HTTP_MSG) then
		...
		if(mg.string.match(mg.str.new(hm:uri()), mg.str.new("/api/video1"))) then
			local conn = mg.connection.new(c);
			conn:data("S", 1);
			mg.printf(conn, string.format("%s", hdr));
		else
			local conn = mg.connection.new(c);
			local opts = mg.http.serve_opts.new("examples/http-client/video-stream/web_root");
			mg.http.serve_dir(conn, ev_data, opts);
		end
	end
end

local function broadcast_mjpeg_frame(arg)
...
local function main(arg)
	...
	local c = mg.http.listen(mgr, "http://localhost:8001", "server_callback");
	mg.timer.add(mgr, 500, MG_TIMER_REPEAT, "timer_callback", mgr);
