...
CALLBACK = function(c, ev, ev_data)
	if(ev == MG_EV_HTTP_MSG) then
		...
		if(mg.string.match(mg.str.ptr(hm.uri), mg.str.ptr("/upload"))) then
			local part = mg.http.part.new();
			local ofs = 0;
			ofs = mg.http.next_multipart(mg.str.ptr(hm.body), ofs, part);

			while(ofs > 0) do
				mg.log.info(string.format("Chunk name: [%s] filename: [%s] length: %u bytes", part.name, part.filename, part.body:len()));
				ofs = mg.http.next_multipart(mg.str.ptr(hm.body), ofs, part);
			end
			...
			local conn = mg.connection.new(c);
			local opts = mg.http.serve_opts.ptr(web_root);
...
local function main(arg)
	...
	mg.http.listen(mgr, s_url, "CALLBACK");
