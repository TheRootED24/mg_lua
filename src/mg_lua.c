...
static int _mg_listen(lua_State *L) {
	...
	lua_State_t *GL = alloc_lua_state(); 
	GL->L = L; // pass the lua_State pointer to fn_serv
	GL->callback = cb;

	mg_connection *conn = (mg_connection*)mg_listen(mgr, s_url, fn, GL);
...
static int _mg_connect(lua_State *L) {
	...
	//mg_connect(mgr, s_url, fn, GL);
	mg_connection *conn = (mg_connection*)mg_connect(mgr, s_url, fn, GL);
...
static int _mg_wakeup(lua_State *L) {
	mg_mgr *mgr = check_mg_mgr(L);
	unsigned long id = (unsigned long)luaL_checklong(L, 2);
	const void *data = (const void*)lua_tostring(L, 3);
	size_t len = (size_t)luaL_checklong(L, 4);
	
	bool ret = mg_wakeup(mgr, id, data, len);
	lua_pushboolean(L, ret);
...
static const luaL_reg mg_methods[] = {
	...
	{"free_lua_states",	free_lua_states },
	{NULL, NULL}
};
