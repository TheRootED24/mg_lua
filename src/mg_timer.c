...
static int _mg_timer_init(lua_State *L) {
	...
	mg_timer *t = check_mg_timer(L, 2);
	uint64_t period_ms = luaL_checknumber(L, 3);
	unsigned flags = luaL_checkint(L, 4);
	const char *cb = luaL_checkstring(L, 5);
...
static int _mg_timer_expired(lua_State *L) {
	uint64_t expiration = luaL_checknumber(L, 1);
	uint64_t period = luaL_checknumber(L, 2);
	uint64_t now = luaL_checknumber(L, 3);
	bool expired  = false;

	expired = mg_timer_expired(&expiration, period, now);
