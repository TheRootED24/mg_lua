...
static int _mg_connection_mgr(lua_State *L) {
	...
	if(NULL != conn->mgr)
		lua_pushlightuserdata(L, conn->mgr);
	else
		lua_pushnil(L);
...
static int _mg_connection_mgr_conns(lua_State *L) {
	...
	if(NULL != conn->mgr->conns)
		lua_pushlightuserdata(L, conn->mgr->conns);
	else
		lua_pushnil(L);
...
static int _mg_connection_recv(lua_State *L) {
	...
	else if(nargs == 3) { // TODO this should use iobuf api (add/del ..etc) instead of directly accessing the buffer.
		if(strcmp(luaL_checkstring(L, 2), "len") == 0)
			conn->recv.len = (size_t)luaL_checkinteger(L, 3);
	}
	else
	{
		lua_pushlstring(L, (const char*)io.buf, io.len);
...
static int _mg_connection_data(lua_State *L) {
	int nargs = lua_gettop(L);
	mg_connection *conn = check_mg_connection(L, 1);
	size_t maxlen = sizeof(conn->data), len = 0;
	// new_index
	if(nargs > 1) {
		char *data = NULL;
		if(nargs > 2) {
...
static int _fn_data(lua_State *L) {
	int nargs = lua_gettop(L);
	mg_connection *conn = check_mg_connection(L, 1);

	if(nargs > 1) {
		conn->fn_data = (void*)lua_topointer(L, 1);

		return 0;
	}

	lua_pushlightuserdata(L, conn->fn_data);
...
static int _mg_connection_new_index(lua_State *L) {
	if(lua_istable(L, 1)) {
		...
		if(lua_istable(L, 1) && (key && strcmp(key, "ctx") != 0)) {
			...
			else if(key && strcmp(key, "is_draining") == 0 ) {
				_is_draininig(L);
			}
			...
			else if(key && strcmp(key, "fn_data") == 0 ) {
				_fn_data(L);
			}
