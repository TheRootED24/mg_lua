...
static int _mg_mqtt_connect(lua_State *L) {
	...
	mg_connection *conn =  mg_mqtt_connect(mgr, s_url, opts, fn, GL);
	if(conn) {
		lua_settop(L, 0);
...
static int _mg_mqtt_listen(lua_State *L) {
	...
	mg_mqtt_listen(mgr, s_url, fn, GL);
...
static int _mg_mqtt_send_header(lua_State *L) {
	mg_connection *conn = check_mg_connection(L, 1);
	uint8_t cmd = (uint8_t)luaL_checkinteger(L, 2);
	uint8_t flags = (uint8_t)luaL_checkinteger(L, 3);
...
static int _mg_mqtt_parse(lua_State *L) {
	mqtt_message *msg = check_mqtt_message(L, 1); //(mqtt_message*)lua_topointer(L, 1);
