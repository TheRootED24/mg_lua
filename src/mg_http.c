...
static int _mg_http_serve_dir(lua_State *L) {
	...
	const http_serve_opts *opts = NULL;

	conn = check_mg_connection(L, 1);
	opts = (const http_serve_opts*)check_mg_http_serve_opts(L, 3);

	// must be cast to mg_http_message* in any case
	if(lua_istable(L, 2)) {
...
static int _mg_http_get_creds(lua_State *L) {
	http_message *hm = check_mg_http_message(L, 1);
	...
	if(ulen > 0) 
		lua_pushlstring(L, user, ulen);
	else
		lua_pushnil(L);

	if(plen > 0)
		lua_pushlstring(L, pass, plen);
	else
		lua_pushnil(L);
...
static int _mg_http_bauth(lua_State *L) {
	mg_connection *c = check_mg_connection(L, 1);
...
static const struct luaL_reg mg_http_lib_m [] = {
	...
	{"creds",		_mg_http_get_creds 	},
	{"bauth",		_mg_http_bauth		},
	{NULL, NULL}
};
